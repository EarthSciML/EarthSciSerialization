...
| **Go** (`esm-format-go`) | **Stubbed.** v0.2.0 always emits `E_NO_DAE_SUPPORT` on mixed-DAE input. Go has no in-ecosystem DAE assembler; the plan is either to vendor a Julia/Rust DAE solver via FFI or to defer DAE support to a future major release. | Deferred (TBD). | Env `ESM_DAE_SUPPORT=0`. |
| **TypeScript** (`earthsci-toolkit`) | **Trivial-factor + error otherwise** (gt-tuva). `discretize()` preprocesses algebraic equations of the form `y ~ expr` (where `y` does not appear in `expr`) by symbolically substituting them into every downstream equation, iterating to a fixed point. If the resulting model is pure-ODE, `metadata.system_class = "ode"` and the output is returned. If any residual algebraic equation remains (implicit constraint, cyclic observed chain, or observed variable that is also differentiated), the binding aborts with `E_NONTRIVIAL_DAE`. Full DAE support is not planned — there is no usable DAE assembler in the JS/TS ecosystem. | None (trivial factoring only; nothing is handed off). | None. The binding has no DAE mode to disable; a model author whose document only contains trivial algebraic equations (observed-style) will succeed, otherwise fail hard with `E_NONTRIVIAL_DAE`. |
