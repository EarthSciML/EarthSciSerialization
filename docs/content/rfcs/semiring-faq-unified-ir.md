...
The compile-fold-vs-setup-fold distinction is a **provenance sub-tag**, not a
class: a `CONST`/`DISCRETE` leaf whose bytes are inline in the document folds at
compile time; one loaded from an external resource (NetCDF mesh/met, §A.8) folds at
