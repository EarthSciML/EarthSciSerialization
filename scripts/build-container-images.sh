...
COPY packages/earthsci-toolkit/Cargo.toml packages/earthsci-toolkit/Cargo.lock ./packages/earthsci-toolkit/

# Pre-fetch dependencies
WORKDIR /workspace/packages/earthsci-toolkit
RUN mkdir src && echo "fn main() {}" > src/main.rs && \
    cargo build --release && \
    rm -rf src

# Copy source and build optimized binary
COPY packages/earthsci-toolkit ./
...
COPY --from=builder --chown=esm:esm /workspace/packages/earthsci-toolkit/target/release/esm /usr/local/bin/esm
