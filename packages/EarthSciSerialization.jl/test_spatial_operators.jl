...
test_cases = [
    ("grad(u, x)", OpExpr("grad", EarthSciSerialization.Expr[VarExpr("u")], dim="x")),
    ("grad(u, y)", OpExpr("grad", EarthSciSerialization.Expr[VarExpr("u")], dim="y")),
    ("div(v, x)", OpExpr("div", EarthSciSerialization.Expr[VarExpr("v")], dim="x")),
    ("div(v, y)", OpExpr("div", EarthSciSerialization.Expr[VarExpr("v")], dim="y")),
    ("laplacian(u)", OpExpr("laplacian", EarthSciSerialization.Expr[VarExpr("u")])),
]
...
for (desc, expr) in test_cases
    ...
    try
        ...
        result = EarthSciSerialization.esm_to_mtk_expr(expr, var_dict, t)
...
lhs = OpExpr("D", EarthSciSerialization.Expr[VarExpr("u")], wrt="t")
rhs = OpExpr("*", EarthSciSerialization.Expr[
    OpExpr("-", EarthSciSerialization.Expr[VarExpr("k")]),
    OpExpr("laplacian", EarthSciSerialization.Expr[VarExpr("u")])
])
eq = Equation(lhs, rhs)
model = Model(vars, [eq])
