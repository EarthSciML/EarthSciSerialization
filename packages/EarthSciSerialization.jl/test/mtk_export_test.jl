...
@testset "mtk2esm: n-ary min/max (nested)" begin
    ...
    eq = model_dict["equations"][1]
    # Symbolics expands -(expr) as (-1)*expr; find the min arg among rhs args.
    rhs_args = eq["rhs"]["args"]
    inner_min = first(a for a in rhs_args if a isa AbstractDict && get(a, "op", nothing) == "min")
