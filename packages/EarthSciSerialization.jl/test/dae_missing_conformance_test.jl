...
@testset "DAE §12 binding contract fixtures (gt-q7sh)" begin
    ...
    for fx in manifest[:fixtures]
        ...
        @testset "$id" begin
            ...
            for mode_expect in obj[:expect]
                ...
                elseif kind == "error"
                    fresh = EarthSciSerialization._deep_native(input)
                    want_code = String(mode_expect[:code])
                    err = try
                        discretize(fresh; dae_support=dae_support)
                        nothing
                    catch e
                        e
                    end
                    ...
                    if haskey(mode_expect, :message_must_contain)
                        for needle in mode_expect[:message_must_contain]
                            @test occursin(String(needle), err.message)
                        end
                    end
                else
