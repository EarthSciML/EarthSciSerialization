...
# Local helper: apply a precomputed (weights, neighbors) stencil to a numeric
# field. Used here purely as a numeric reference for the symbolic ArrayOp.
# The public `apply_laplacian!` / `apply_gradient!` paths were retired with
# esm-4t5 (single-evaluation-pathway directive); the production code only
# applies stencils symbolically through `fv_*_extended` ArrayOps.
...
@testset "grid_assembly_symbolic: fv_laplacian_extended matches numeric stencil apply" begin
    g = SymTestCartesianGrid(16, 16, 2π, 2π)
    xs = cell_centers(g, :x); ys = cell_centers(g, :y)
    u = @. sin(xs) * cos(ys)

    # Numeric reference: precompute + local apply helper.
...
@testset "grid_assembly_symbolic: fv_gradient_extended matches numeric stencil apply" begin
