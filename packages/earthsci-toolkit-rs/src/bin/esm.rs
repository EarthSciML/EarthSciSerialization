...
fn generate_julia_code(esm_file: &earthsci_toolkit::EsmFile) -> String {
                    ...
                    if node.op == "d" && node.args.len() >= 1 {
                        if let earthsci_toolkit::Expr::Variable(var_name) = &node.args[0] {
                                ...
                                model.variables.keys().position(|v| v == var_name)
                            {
                                ...
                                code.push_str(&format!(
                                    "    # Unknown variable in derivative: d{}/dt = {}\n",
                                    var_name, rhs
                                ));
                            }
                        }
    ...
    if let Some(ref reaction_systems) = esm_file.reaction_systems {
        for (rs_id, rs) in reaction_systems {
            ...
            // Species mapping (sorted for stable ordering)
...
fn generate_python_code(esm_file: &earthsci_toolkit::EsmFile) -> String {
    ...
    if let Some(ref reaction_systems) = esm_file.reaction_systems {
        for (rs_id, rs) in reaction_systems {
            ...
            // Species unpacking (sorted for stable ordering)
            let mut species_names: Vec<&String> = rs.species.keys().collect();
            species_names.sort();
            for (i, species_name) in species_names.iter().enumerate() {
                code.push_str(&format!(
...
fn analyze_expression_optimization(esm_file: &earthsci_toolkit::EsmFile) {
    ...
    if suggestions.is_empty() {
        ...
        println!(
            "Found {} complex expressions, {} redundant operations",
            complex_expressions, redundant_operations
        );
...
fn analyze_structure_optimization(esm_file: &earthsci_toolkit::EsmFile) {
    ...
    println!(
        "Structure: {} models, {} reaction systems, {} coupling rules",
    ...
    if let Some(ref models) = esm_file.models {
        for (model_id, model) in models {
            ...
            if var_count > 50 {
                suggestions.push(format!(
                    "Model '{}' has {} variables - consider splitting into smaller components",
                    model_id, var_count
                ));
            }
            ...
            if eq_count > 30 {
                suggestions.push(format!(
                    "Model '{}' has {} equations - consider modular decomposition",
                    model_id, eq_count
                ));
            }
            ...
            if unused_count > 0 {
                suggestions.push(format!(
                    "Model '{}' has {} potentially unused variables",
                    model_id, unused_count
                ));
            }
        }
    }
...
fn analyze_performance_optimization(esm_file: &earthsci_toolkit::EsmFile) {
    ...
    if expensive_ops > 5 {
        println!(
            "  - {} expensive operations found - consider lookup tables or approximations",
            expensive_ops
        );
    }
...
fn perform_structural_comparison(
    ...
    if coupling1_count != coupling2_count {
        differences.push(format!(
            "Coupling rule count: {} vs {}",
            coupling1_count, coupling2_count
        ));
    }
...
fn perform_numerical_comparison(
            ...
            if let Some(system2) = rs2.get(rs_id) {
                ...
                if system1.reactions.len() == system2.reactions.len() {
                    for (i, (reaction1, reaction2)) in system1
                        ...
                        .enumerate()
                    {
                        let subs1 = reaction1.substrates.as_deref().unwrap_or(&[]);
                        let subs2 = reaction2.substrates.as_deref().unwrap_or(&[]);
                        // Compare substrate coefficients
                        if subs1.len() == subs2.len() {
                            for (sub1, sub2) in subs1.iter().zip(subs2.iter()) {
                                let coeff1 = sub1.coefficient as f64;
                                let coeff2 = sub2.coefficient as f64;
                        ...
                        let prods1 = reaction1.products.as_deref().unwrap_or(&[]);
                        let prods2 = reaction2.products.as_deref().unwrap_or(&[]);
                        // Compare product coefficients
                        if prods1.len() == prods2.len() {
                            for (prod1, prod2) in prods1.iter().zip(prods2.iter()) {
                                let coeff1 = prod1.coefficient as f64;
                                let coeff2 = prod2.coefficient as f64;
                                if (coeff1 - coeff2).abs() > tolerance {
    ...
    if differences.is_empty() {
        println!(
            "✓ Files are numerically equivalent (tolerance: {})",
            tolerance
        );
    } else {
...
fn count_numerical_values(esm_file: &earthsci_toolkit::EsmFile, count: &mut usize) {
    ...
    if let Some(ref reaction_systems) = esm_file.reaction_systems {
        for (_, rs) in reaction_systems {
            for reaction in &rs.reactions {
                // Count coefficients
                for _substrate in reaction.substrates.iter().flatten() {
                    *count += 1;
                }
                for _product in reaction.products.iter().flatten() {
                    *count += 1;
                }
...
fn perform_deep_coupling_analysis(esm_file: &earthsci_toolkit::EsmFile) {
    ...
    if *max_connections > 5 {
        println!(
            "- Hub components detected (max {} connections) - consider architectural refactoring",
            max_connections
        );
    ...
    println!(
        "\nCoupling density: {:.3} (0=no coupling, 1=fully coupled)",
        coupling_density
    );
...
fn analyze_coupling_patterns(graph: &earthsci_toolkit::graph::ComponentGraph) {
    ...
    if chain_length > 4 {
        println!(
            "Long dependency chain detected: {} components",
            chain_length
        );
        ...
        for rule in coupling {
            match rule {
                ...
                earthsci_toolkit::CouplingEntry::Event { name, affects, .. } => {
                    ...
                    println!(
                        "  Event coupling: {} affects {} components",
                        event_name, affected_count
                    );
                }
            }
        }
...
fn detect_coupling_antipatterns(
    ...
    for (pair, count) in &component_pairs {
        if *count > 2 {
            antipatterns_found.push(format!(
                "Inappropriate intimacy: {} ({} coupling types)",
                pair, count
            ));
        }
    }
    ...
    match cli.command {
        ...
        Commands::Stoich { file, system } => {
            ...
            if let Some(ref reaction_systems) = esm_file.reaction_systems {
                if let Some(rs) = reaction_systems.get(&system) {
                    ...
                    // Create species index map (sorted for stable column order)
                    let mut species_names: Vec<&String> = rs.species.keys().collect();
                    species_names.sort();
            ...
            match level.as_str() {
                ...
                "expression" => {
                    ...
                    if let Some(ref _sys) = system {
                        eprintln!(
                            "Expression-level graphs for system '{}' not yet implemented",
                            _sys
                        );
                    } else {
                ...
                _ => {
                    eprintln!(
                        "Unsupported graph level: {}. Use component or expression.",
                        level
                    );
                    std::process::exit(1);
                }
        ...
        Commands::Convert { input, output, to } => {
            ...
            let output_content = match to.as_str() {
                ...
                _ => {
                    eprintln!(
                        "Unsupported format: {}. Use json, compact-json, julia, or python.",
                        to
                    );
                    std::process::exit(1);
                }
        ...
        Commands::Analyze {
            ...
            match analysis_type.as_str() {
                "all" | "structure" => {
                    ...
                    println!(
                        "Components: {} models, {} reaction systems, {} data loaders, {} operators",
                    ...
                    println!(
                        "Total: {} variables, {} equations, {} species, {} reactions",
        ...
        Commands::PerformanceProfile { file, profile_type } => {
            ...
            match profile_type.as_str() {
                ...
                _ => {
                    eprintln!(
                        "Unsupported profile type: {}. Use memory, time, or both.",
                        profile_type
                    );
                    std::process::exit(1);
                }
        ...
        Commands::Compare {
            ...
            match comparison_type.as_str() {
                ...
                _ => {
                    eprintln!(
                        "Unsupported comparison type: {}. Use semantic, structural, or numerical.",
                        comparison_type
                    );
                    std::process::exit(1);
                }
      ...
      "transform": {"op": "*", "args": ["temperature", 0.1]}
        ...
        Commands::Benchmark {
            ...
            match bench_type.as_str() {
                ...
                _ => {
                    eprintln!(
                        "Unsupported benchmark type: {}. Use all, parse, validate, or simulate.",
                        bench_type
                    );
                    std::process::exit(1);
                }
