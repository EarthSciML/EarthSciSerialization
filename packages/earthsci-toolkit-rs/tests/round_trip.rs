...
/// Round-trip for the `expression` initial-condition golden (bead ess-gjn,
/// epic campfire-e2e ea-6sh). The domain declares an `expression` initial
/// condition: a map from variable to an Expression AST evaluated over the
/// spatial grid at t=0 to produce u(x, 0). Rust stores the domain
/// `initial_conditions` as an untyped JSON value, so this locks the shared
/// cross-binding contract — the expression IC parses and survives
/// load -> save -> load verbatim. End-to-end expression-IC simulation in Rust
/// is deferred (Python-first per ess-gjn); like the `aggregate` arrayop
/// fixtures, this method-of-lines fixture is exercised by load/round-trip
/// rather than the structural validator (which does not yet count arrayop
/// equations as ODEs).
#[test]
fn test_expression_initial_condition_round_trip() {
    let fixture =
        include_str!("../../../tests/valid/initial_conditions/expression_ignition_front_1d.esm");

    let parsed: EsmFile = load(fixture).expect("Failed to parse expression-IC golden");

    // The domain expression initial condition is present after parse.
    let ic = parsed
        .domains
        .as_ref()
        .and_then(|d| d.get("line"))
        .and_then(|dom| dom.initial_conditions.as_ref())
        .expect("domain `line` initial_conditions present");
    assert_eq!(ic.get("type").and_then(|v| v.as_str()), Some("expression"));

    // Round-trip: the expression IC survives load -> save -> load verbatim.
    let serialized = save(&parsed).expect("Failed to serialize expression-IC golden");
    let reparsed: EsmFile = load(&serialized).expect("Failed to reparse expression-IC golden");
    assert_eq!(parsed.esm, reparsed.esm);
    let reic = reparsed
        .domains
        .as_ref()
        .and_then(|d| d.get("line"))
        .and_then(|dom| dom.initial_conditions.clone())
        .expect("domain `line` initial_conditions present after round-trip");
    assert_eq!(&reic, ic);
}
