...
export const CouplingGraphDemo = () => {
    const [selectedComponent, setSelectedComponent] = createSignal(null);
    const [editingCoupling, setEditingCoupling] = createSignal(null);
    ...
    const handleEditCoupling = (coupling, edgeId) => {
    ...
    return (<div style={{ padding: '20px', font: '14px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Oxygen, Ubuntu, Cantarell, sans-serif' }}>
      ...
      <div style={{ border: '1px solid #ddd', 'border-radius': '8px', overflow: 'hidden' }}>
        <CouplingGraph esmFile={sampleEsmFile} onSelectComponent={handleSelectComponent} onEditCoupling={handleEditCoupling} width={1000} height={700} interactive={true}/>
      </div>
      ...
      <div style={{ margin: '20px 0', display: 'grid', 'grid-template-columns': '1fr 1fr', gap: '20px' }}>
        <div>
          ...
          <div style={{
            ...
            {selectedComponent() ? (<div>
              ...
              </div>) : (<p style={{ margin: 0, color: '#666' }}>No component selected</p>)}
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{
            ...
            {logMessages().length > 0 ? (<div style={{ 'font-family': 'monospace', 'font-size': '11px' }}>
                {logMessages().map((msg, i) => (<div key={i} style={{ margin: '2px 0', color: '#555' }}>{msg}</div>))}
              </div>) : (<p style={{ margin: 0, color: '#666' }}>No activity yet</p>)}
          </div>
        </div>
      </div>

      {/* Coupling Editor Modal */}
      {editingCoupling() && (<div style={{
                position: 'fixed',
                top: 0,
                left: 0,
                right: 0,
          ...
          <div style={{
            ...
            <div style={{ margin: '20px 0 0 0', 'text-align': 'right' }}>
              <button onClick={closeCouplingEditor} style={{
                background: '#0078d4',
                color: 'white',
                border: 'none',
                padding: '8px 16px',
        ...
        <div style={{ display: 'grid', 'grid-template-columns': 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
          <div style={{ padding: '12px', background: '#e6f3ff', 'border-radius': '6px' }}>
            ...
            <ul style={{ margin: '8px 0 0 0', 'padding-left': '20px', 'font-size': '13px' }}>
              {Object.keys(sampleEsmFile.models || {}).map(name => (<li key={name}>{name}</li>))}
            </ul>
          </div>
          ...
          <div style={{ padding: '12px', background: '#fff0e6', 'border-radius': '6px' }}>
            ...
            <ul style={{ margin: '8px 0 0 0', 'padding-left': '20px', 'font-size': '13px' }}>
              {Object.keys(sampleEsmFile.reaction_systems || {}).map(name => (<li key={name}>{name}</li>))}
            </ul>
          </div>
          ...
          <div style={{ padding: '12px', background: '#f0f9e6', 'border-radius': '6px' }}>
            ...
            <ul style={{ margin: '8px 0 0 0', 'padding-left': '20px', 'font-size': '13px' }}>
              {Object.keys(sampleEsmFile.data_loaders || {}).map(name => (<li key={name}>{name}</li>))}
            </ul>
          </div>
          ...
          <div style={{ padding: '12px', background: '#f9e6ff', 'border-radius': '6px' }}>
            ...
            <ul style={{ margin: '8px 0 0 0', 'padding-left': '20px', 'font-size': '13px' }}>
              {Object.keys(sampleEsmFile.operators || {}).map(name => (<li key={name}>{name}</li>))}
            </ul>
          </div>
        </div>
...
//# sourceMappingURL=CouplingGraph.demo.jsx.map