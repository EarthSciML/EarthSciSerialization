...
describe('ExpressionNode', () => {
    ...
    const createTestNode = (expr, props = {}) => {
        const [highlightedVars] = createSignal(new Set());
        return render(() => <ExpressionNode expr={expr} path={['test']} highlightedVars={highlightedVars} onHoverVar={mockOnHoverVar} onSelect={mockOnSelect} onReplace={mockOnReplace} allowEditing={true} {...props}/>);
    };
    beforeEach(() => {
        vi.clearAllMocks();
    });
    ...
    describe('Interactive Features', () => {
        ...
        it('should enter edit mode on double-click for numbers', async () => {
            ...
            expect(editInput?.value).toBe('42');
        });
    ...
    describe('CSS Classes and Styling', () => {
        ...
        it('should apply highlighted class for highlighted variables', () => {
            ...
            const { container } = render(() => <ExpressionNode expr="x" path={['test']} highlightedVars={highlightedVars} onHoverVar={mockOnHoverVar} onSelect={mockOnSelect} onReplace={mockOnReplace}/>);
            const nodeElement = container.querySelector('.esm-expression-node');
    ...
    describe('Complex Expressions', () => {
        ...
        it('should handle empty or malformed expressions gracefully', () => {
            ...
            const { container: nullContainer } = createTestNode(null);
            expect(nullContainer.querySelector('.esm-unknown')).toBeTruthy();
            // Test with empty object
            const { container: emptyContainer } = createTestNode({});
            expect(emptyContainer.querySelector('.esm-unknown')).toBeTruthy();
        });
    });
});
//# sourceMappingURL=ExpressionNode.test.jsx.map