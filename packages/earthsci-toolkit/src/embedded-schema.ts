  ...
  "$defs": {
            ...
            "oneOf": [
              ...
              },
              {
                ...
                "properties": {
                  "from": {
                    "type": "string",
                    "description": "Name of a declared index set: a key in Model.index_sets. The resolver MUST error on an undeclared name; no implicit interval is inferred, so a typo cannot silently become an empty set."
                  },
                  "of": {
                    "type": "array",
    ...
    "Model": {
      ...
      "properties": {
        ...
        "index_sets": {
          "type": "object",
          "description": "Document-scoped registry of named index sets (RFC semiring-faq-unified-ir §5.2), keyed by name — the single declaration site for every iteration domain (grid axes, categorical dimensions, data-derived sets). An `aggregate` range references one by name as { \"from\": <name> }. Each entry is an interval (dense axis), categorical enumeration, data-derived set, or ragged / dependent inner set. A reference resolves by name to exactly one entry; resolvers MUST error on an undeclared name.",
          "additionalProperties": {
            "$ref": "#/$defs/IndexSet"
          }
        }
