...
def simulate(
    ...
    # `enum`, `table_lookup`). The SymPy `lambdify` path handles scalar-only
    # systems whose bodies are pure AST math; closed-function bodies (e.g.
    # `gaschem/fastjx.esm`'s `interp.bilinear`) are scalar but still need the
    # numerical AST walker.
    has_array = any(
