...
This guide will walk you through the basics of using ESMFormat.jl.

## Installation

ESMFormat.jl is registered in the Julia General Registry and can be installed using:
...
```julia
using Pkg
Pkg.add(url="https://github.com/EarthSciML/EarthSciSerialization.git", subdir="packages/ESMFormat.jl")
```
...
esm_file = ESMFormat.parse(json_str)
```
...
json_string = ESMFormat.serialize(esm_file)
```
