#!/usr/bin/env python3
"""
Simple test script to verify migration functionality using basic dict-like objects
"""
...
class MockEsmFile:
    ...
    def __init__(self, esm, metadata=None, reaction_systems=None):
        self.esm = esm
        self.metadata = metadata or {}
        self.reaction_systems = reaction_systems or {}

class MockSpecies:
    """Mock Species for testing"""
    def __init__(self, units, default, description=""):
        self.units = units
        self.default = default
        self.description = description

class MockReactionSystem:
    """Mock ReactionSystem for testing"""
...
try:
    ...
    metadata = MockMetadata(title="Test Model", description="Test migration functionality")

    # Create species for testing
    ch4_species = MockSpecies(units="ppbv", default=1900.0, description="Methane")
    co_species = MockSpecies(units="ppmv", default=0.2, description="Carbon monoxide")

    # Create reaction system
    test_system = MockReactionSystem(species={"CH4": ch4_species, "CO": co_species})

    # Create the ESM file
    test_file = MockEsmFile(
        esm="0.0.5",
        metadata=metadata,
    ...
    try:
        ...
        if migrated_ch4.units == "mol/mol" and abs(migrated_ch4.default - 1.9e-6) < 1e-10:
            ...
            print(f"  Expected: mol/mol (1.9e-06), Got: {migrated_ch4.units} ({migrated_ch4.default})")

        # Check migration notes
        if hasattr(migrated.metadata, 'migration_notes'):
            print("✓ Migration notes field exists")
        elif "Migration notes: Migrated from version 0.0.5" in str(migrated.metadata.description):
            print("✓ Migration notes added to description")
        else:
            print("✗ Migration notes missing")
            print(f"  Metadata description: {migrated.metadata.description}")
